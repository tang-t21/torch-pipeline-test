# LLM Pipeline Parallel Test Bench
...
The schedule receives the whole batch and splits it internally into microbatches. This is important because it keeps the user-facing API close to a normal training step while still exercising pipeline scheduling behavior.
...
When `--profile` is enabled, `torch.profiler` traces are written under:

- `traces/<run_name>/rankXX/`
...
## Running On The Provided Kubernetes GPU Pod
